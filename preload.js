...
contextBridge.exposeInMainWorld("gideon", {
  ...
  fetchAttachment: (uid, name, folder) => ipcRenderer.invoke("fetch-attachment", uid, name, folder),
  sendMail:        (opts)       => ipcRenderer.invoke("send-mail", opts),
  deleteMessage:   (uid, folder) => ipcRenderer.invoke("delete-message", uid, folder),
  toggleFlag:      (uid, flag, folder) => ipcRenderer.invoke("toggle-flag", uid, flag, folder),
  ...
  gcalCheckConnection: ()     => ipcRenderer.invoke("gcal-check-connection"),
  gcalGetEvents:     (timeMin, timeMax) => ipcRenderer.invoke("gcal-get-events", timeMin, timeMax),
  calendarCandidatesScan: (messages) => ipcRenderer.invoke("calendar-candidates-scan", messages),
  calendarCandidateResolve: (uid, date) => ipcRenderer.invoke("calendar-candidate-resolve", uid, date),
  gcalDeleteEvent: (eventId) => ipcRenderer.invoke("gcal-delete-event", eventId),
